...
**Auto-step for the "click runs but workflow no-ops" symptom (Ori 643ca02e, 2026-05-17; refined Ori 64dc161d; premise-verification added Ori f75090bf, 2026-05-18):** if a sub-agent reports a click that returns `body_changed:false` AND `dom_event_listeners` shows listeners on the target AND no popup/dialog/error appears, take ALL THREE of these steps in order, BEFORE escalating to another click variant or a platform-primitive proposal:

0. **PREMISE VERIFICATION — trace the option-key mapping (Ori f75090bf, 2026-05-18).** When the Conditional State reads `option:X:____N`, do NOT infer N from display order. Bubble encodes option keys in CREATION order (`____0` = first-created, regardless of where it appears in the option set UI), and the export tooling preserves these opaque indices without resolving them to display labels. Three cheap ways to resolve the mapping, any one is sufficient: (a) PRD lookup — status fields are usually documented by name (one grep beats five click probes); (b) trace the writer — find the workflow Action that sets the field (`ChangeListOfThings` / "Make changes to a thing") and read its target `option:X:____N`; (c) runtime label trace — `bubble.option_sets["X"]["____N"].get("display")` returns the localized label if the option set is reachable. **Cost of this step: 2 minutes. Cost of skipping it: a week of platform-investigation work on a wrong premise.**

1. **Check the element's computed cursor** (cheapest click-side diagnostic). One `evaluate` call against the locator: `getComputedStyle(el).cursor` plus the inline-style `cursor:`. If the cursor is `default` (not `pointer`), the runtime gate has already decided the button is disabled — no click variant will fix it. The answer is at the data / conditional-state layer, not the click layer. STOP click escalation; investigate the underlying field that drives the conditional state (run step 0 if you haven't).

2. **Read the source meta** (~3 min). For Bubble apps, grep the trigger element's path for `Button Disabled:` + `Conditional States` in `_index.md`. The source tells you which data fields the conditional reads. **Bubble's runtime conditional evaluation matches the export exactly** — if your source-stated condition appears satisfied but the cursor says disabled, you've probably misread the option-key mapping (re-run step 0). The earlier "runtime DIVERGES from source export" framing here is SUPERSEDED by f75090bf: no divergence was ever observed; the divergence-impression came from an inverted option-key mapping.

Ori K3 (2026-05-17, SUPERSEDED 2026-05-18 by f75090bf): 371K tokens of sub-agent retries + 5 medium-severity issue threads (`06f41145`, `643ca02e`, `64dc161d`, `d44746db`, `38247b33`) all rested on assuming `____0` = first display value (Draft). It actually = last-created value (Closed/נסגר). The K3 test's own block 6 `close_arrangement` set the pick to ____0 before block 9 tried to click Edit — the button was correctly disabled, doing exactly what the source said. No platform fix needed; no runtime divergence ever existed. ~2 minutes of PRD-reading (or workflow-trace via step 0) would have reversed the verdict.
...
Ori K3 worked example (probe9, 2026-05-17, SUPERSEDED 2026-05-18 by f75090bf): all 7 probes (CDP isTrusted, pointer-sequence hold_ms:80, force-strip inline cursor, jQuery .trigger("click"), dblclick+keyboard, direct handler call, full press sequence) reached the gate but yielded no popup. The original framing — "decisive evidence the class is structurally inaccessible from user JS" — is walked back: the probes correctly observed that the click was being blocked, by the legitimate Conditional-State gate on a pick the test had just intentionally closed (block 6 of K3 = `close_arrangement` → status = ____0 = "Closed"; block 9 = click Edit on a Closed pick → button correctly disabled). The ladder itself is still useful as a fast-fail confirmation for genuine Button-Disabled-with-Conditional-State gates, but always run step 0 (premise verification) first — if you reach D in the ladder before verifying your option-key mapping, you may be wrongly concluding "structural gate" when the real answer is "the test ran a writer that disabled the button on itself."
