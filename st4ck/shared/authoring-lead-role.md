...
| `qa-runner` | After sign — one per test (or one per batch). Drives `@st4ck/runner` (`npx st4ck-runner run`) and handles agentic-block IPC pauses inline. | `{outcome, results: [...], totals, stop_reason?}`. |
