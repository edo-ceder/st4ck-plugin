...
Before fixing, reproduce via `/st4ck:browse` (or one Bash call per primitive — `npx st4ck@latest browse launch <url> --session debug-<slug>` then `… browse <op> --session debug-<slug>` per step):

1. Get the fixture page / session URL from the log.
2. Open a runner session against that URL with `/st4ck:browse` (or `npx st4ck@latest browse launch …` directly).
