...
3. Fill in the "Components" column in the Test Journeys table for each journey
...
1. **Internal consistency** — Do sections reference each other correctly? If Phase 2 mentions a table created in Phase 1, does Phase 1 actually create it? If a test journey references a component, is it listed in the Components table? If a task targets a file, is that file in the Relevant Files table?

2. **Integration paths complete** — For each new data format or API, trace the path through all layers that touch it (schema → handler → validation → DB → consumer). Does every layer have a task? If Phase 3 adds a new field to a schema, does another phase add code that sets it?

3. **Cross-references survive edits** — Adding or moving a phase renumbers everything downstream. Check that task IDs, test journey IDs, and phase references still point to the right targets.

4. **Specificity** — Read each acceptance criterion. Could an agent implement it without asking a question? Read each expected result. Could a QA author write a test from it without guessing?

5. **Displacement damage** — Did this pass's fixes move or change things that break other sections? Check the areas adjacent to every edit.
...
- [ ] Context for Reviewers section is filled in (app summary, current/desired state, why)
