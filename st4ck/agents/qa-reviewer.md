...
### Seal Rules
- Component `eval_sequence` change → component `review_signature` breaks → component needs re-review via `sign_component_review`. Test `journey_signature` is NOT affected — only the component itself needs re-signing.
